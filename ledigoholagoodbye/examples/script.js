...
HaxballJS().then((HBInit) => {
  try {
    room = HBInit({
      ...
      token: "thr1.AAAAAGfnWs1KIxG5D-e2ug.4aPU8AP9PbY"
    });
    // | 𝘓𝘌𝘎𝘐𝘖𝘕 𝘗𝘈𝘕𝘋𝘈 - 🐼🎋
    // 𝐉𝐔𝐄𝐆𝐀𝐍 𝐓𝐎𝐃𝐎𝐒 | 𝐏𝐀𝐍𝐃𝐀🐼🎋
    const ranks = {
    ...
    const warnedPlayers = {};
    const afkTimestamps = {};
    const multicolors = [0xfe0000, 0xf24336, 0xe35f5d, 0xcf7584, 0xb388ac, 0x8899d4, 0x00a9fe];
    ...
    const MODES = ['power', 'comba', 'normal'];
    const BOOST_SPEEDS = [1.4, 1.6, 1.8, 2];
    ...
    function startGame() {
      ...
      if (currentMode === 'power') {
        ...
        gravityEnabled = true;
        room.sendAnnouncement('Este partido se juega con: 🤑🤩COMBA🤩🤑.', null, 0xff7759, "bold", 2);
      } else if (currentMode === 'normal') {
        gravityEnabled = false;
        powerEnabled = false;
        room.sendAnnouncement('Este partido se juega SIN POWER NI COMBA', null, 0xff7759, "bold", 2);
      }
    }
    ...
    function sendJoin(playerName, playerAuth, playerConn, playerIp) {
      if (!playerName || !playerAuth || !playerConn || !playerIp || playerAuth === rolesData.roles["coowner"].users.includes(playerAuth)) {
        return;
      }
    ...
    room.onPositionsReset = function () {
      ...
      if (ballColorChanged && ballColor !== null) {
        color = ballColor;
      } else {
    ...
    room.onTeamGoal = (team) => {
      ...
      if (team === 1) {
        Marcador.Red++;
        if (!ballColorChanged && !multicolorActive) room.setDiscProperties(0, { color: 0xFF0000, radius: 6.25 * 3 });
      } else if (team === 2) {
        Marcador.Blue++;
        if (!ballColorChanged && !multicolorActive) room.setDiscProperties(0, { color: 0x0000FF, radius: 6.25 * 3 });
      }
      ...
      if (message.startsWith("!sancionar")) {
        ...
        if (playerStats[playerAuth]) {
          if (!playerStats[playerAuth].verified) {
            ...
            room.sendAnnouncement(`🚀 Power activado: ${powerEnabled ? "SI" : "NO"}\n🌠 Comba activada: ${gravityEnabled ? "SI" : "NO"}`, player.id, 0xbaa050, "small-bold", 1);
          }
        }
        return false;
        ...
        if (playerStats[playerAuth]) {
          if (!playerStats[playerAuth].verified) {
            ...
            room.sendAnnouncement("💎 !ballcolor [0xcolor] o [multicolor] - Cambia el color de la pelota. Se reinicia al terminar el partido.", player.id, 0x52a489, "bold", 2);
          }
        }
        return false;
        ...
        return false;
      } else if (message.startsWith("!ballcolor")) {
        const args = message.split(" ");
        const hexColorRegex = /^0x[0-9A-Fa-f]{6}$/;

        let color = args[1]?.trim();
        ...
        if (!color) {
          room.sendAnnouncement("Debes especificar un color. Ejemplo: !ballcolor 0xFF5733 o !ballcolor multicolor", player.id, 0x72c1db, "bold", 2);
          return false;
        }

        if (!rolesData.roles["vips"].users.includes(playerAuth)) {
          room.sendAnnouncement("Comando solo para VIPS.", player.id, 0xFF0000, "bold", 2);
          return false;
        }
        ...
        if (color.toLowerCase() === "multicolor") {
          if (multicolorActive) {
            room.sendAnnouncement("El multicolor ya está activo", player.id, 0xFF0000, "bold", 2);
            return false;
          }
          ...
          room.sendAnnouncement("🌈 Pelota cambiada a multicolor", null, 0x00FF00, "bold", 2);
          return false;
        }
        ...
        room.setDiscProperties(0, { color: numericColor });

        ballColor = numericColor;
        ballColorChanged = true;

        room.sendAnnouncement(`🎨 Color de la pelota cambiado a ${color}`, null, 0x00FF00, "bold", 2);
        return false;
      } else if (message.startsWith("!ruleta")) { // Final de la tienda
        const args = message.split(' ');
      ...
      if (powerEnabled) {
        ...
        if (closestPlayer) {
          if (ballHeldBy === null || ballHeldBy !== closestPlayer.id) {
            ...
            powerActive = false;
            powerLevel = -1;
            if (!multicolorActive && !ballColorChanged) room.setDiscProperties(0, { color: NORMAL_BALL_COLOR });
    ...
    setInterval(() => {
      if (!powerActive && !gravityActive) {
        if (multicolorActive) {
          const nextIndex = (multicolorIndex + 1) % multicolors.length;
          const newColor = interpolateColor(multicolors[multicolorIndex], multicolors[nextIndex], transitionProgress);
